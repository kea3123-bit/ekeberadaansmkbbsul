// ---------- Admin data / report ----------
  ...
  // ---------- Settings / visual map ----------
  function fillSettings(s){const f=document.getElementById('settingsForm');if(!f)return;Object.keys(s).forEach(k=>{if(f.elements[k])f.elements[k].value=s[k]??'';});updateModeSettingHint();updateGpsSettingWarning();if(state.locationMap)setTimeout(syncMapFromInputs,30);}
